...
async function main() {
    ...
    const resultados = xlsx.utils.sheet_to_json(worksheet, { header: 1 });
    ...
    for (let i = 0; i < resultados.length; i++) {
        ...
        // Encontra coluna do "Horários" na linhaSeguinte (pode ser 0, 1, 2...)
        const indexHorarios = linhaSeguinte.indexOf("Horários");
        
        if (linhaAtual && indexHorarios !== -1 && linhaAtual[indexHorarios]) {
            const nomeRota = String(linhaAtual[indexHorarios]).trim();
            ...
            for (let index = 0; index < cidadesStr.length; index++) {
                const cidade = cidadesStr[index] || "Desconhecida";
                const base = await prisma.base.upsert({
                    where: { nome: cidade },
                    update: {},
                    create: { nome: cidade, cidade: cidade, raioMetros: 500 },
                });
                ...
                console.log(`  -> Parada ${index}: ${cidade} | Chegada: ${horaChegada || "--"} | Saída: ${horaSaida || "--"}`);
            }
            rotasCriadas++;
        }
    }
